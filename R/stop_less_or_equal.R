...
#' x <- 1:2
#' # Should not call stop_less_or_equal() in the folowing one
#' stop_less_or_equal(x, c(0, 3), mod = "any") |> try()
#' # ...but for the next one
#' stop_less_or_equal(x, c(0, 3), mod = "all") |> try()
stop_less_or_equal <- function(lhs, rhs, ..., mod = NULL, na.rm = FALSE,
...
#' @rdname stop_less_or_equal
#' @export
`stop_<=` <- stop_less_or_equal
